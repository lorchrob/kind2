...
let rec minimize_node_call_args ue lst expr =
  ...
  let rec aux expr =
    ...
    | A.StructUpdate (p,e1,ls,Some e2,ta) -> A.StructUpdate (p,aux e1,ls,Some (aux e2),ta)
    | A.StructUpdate (p,e1,ls,None,ta) -> A.StructUpdate (p,aux e1,ls,None,ta)
    ...
    | A.Arrow (p,e1,e2,ta) -> A.Arrow (p,aux e1,aux e2,ta)
...
and ast_contains p ast =
  let rec aux ast =
    ...
    | A.ConvOp (_,_,e) | A.UnaryOp (_,_,e) | A.RecordProject (_,e,_)
      ...
      | A.When (_,e,_) | A.Pre (_,e,_) | A.StructUpdate (_, e, _, None, _) 
      | A.ChooseOp (_,_,e) | A.AnyOp (_,_,e) | A.Extract (_,e,_,_) ->
      aux e
    | A.StructUpdate (_,e1,_,Some e2,_) | A.ArrayConstr (_,e1,e2)
    | A.IndexAccess (_,e1,e2,_) 
    | A.BinaryOp (_,_,e1,e2) | A.CompOp (_,_,e1,e2)
    | A.Arrow (_,e1,e2,_) -> aux e1 || aux e2
    | A.GroupExpr (_,_,es) ->
      List.map aux es
      |> List.exists (fun x -> x)
