...
let cmd_line
  ...
  (* Path and name of Vampire executable *)
  let vampire_bin = Flags.Smt.vampire_bin () in
...
(* Command to limit check-sat in Vampire to run for the given numer of ms
   at most *)
...
let headers minimize_cores =
  (* Core minimization only supported by Vampire for now *)
