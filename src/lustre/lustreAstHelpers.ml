...
let rec map_lustre_ty f ty = 
  ...
  | ADT (p, id, k, cons) ->
    ADT (p, id, k, List.map (fun (id, tys) -> id, List.map r tys) cons)
...
let rec pat_bound_vars = function
  | Pat (_, id, _, []) -> SI.singleton id
  | Pat (_, _, _, sub_pats) -> SI.flatten (List.map pat_bound_vars sub_pats)
...
let rec substitute_naive (var:HString.t) t = function
  ...
  | ADTTerm (pos, ctor, k, args) ->
    ADTTerm (pos, ctor, k, List.map (substitute_naive var t) args)
...
let rec apply_subst_in_expr sigma = function
  ...
  | ADTTerm (pos, ctor, k, args) ->
    ADTTerm (pos, ctor, k, List.map (apply_subst_in_expr sigma) args)
  | RecordExpr (pos, ident, ps, expr_list) ->
...
= fun sigma expr -> match expr with
  ...
  | Match _ -> assert false (* Not supported due to introduction of bound variables *)
  | ADTTerm (pos, ctor, k, args) ->
    ADTTerm (pos, ctor, k, List.map (apply_type_subst_in_expr sigma) args)
  | Ident _
  | ModeRef _  -> expr
...
= fun sigma ty -> match ty with
  | UserType (pos, ty_args, i, k) -> (
    match List.assoc_opt i sigma with
      | Some ty -> ty
      | None ->
        UserType (pos, List.map (apply_type_subst_in_type sigma) ty_args, i, k)
  )
...
let rec replace_with_constants: expr -> expr =
  ...
  | ADTTerm (pos, ctor, k, args) ->
    ADTTerm (pos, ctor, k, List.map replace_with_constants args)
...
let rec abstract_pre_subexpressions: expr -> expr = function
  ...
  | ADTTerm (pos, ctor, k, args) ->
    ADTTerm (pos, ctor, k, List.map abstract_pre_subexpressions args)
...
let rec replace_idents locals1 locals2 expr = 
  ...
  | ADTTerm (pos, ctor, k, args) ->
    ADTTerm (pos, ctor, k, List.map r args)
...
and syn_type_equal depth_limit x y : (bool, unit) result =
  ...
  let rec r depth x y =
    ...
    | IntRange (_, xe1, xe2), IntRange (_, ye1, ye2) ->
      ...
      in
      Ok (e1 && e2)
    | UserType (_, ty_args1, x, _), UserType (_, ty_args2, y, _) ->
...
let rec rename_contract_vars = function
  ...
  | ADTTerm (pos, ctor, k, args) ->
    ADTTerm (pos, ctor, k, List.map rename_contract_vars args)
...
let pos_of_type ty = match ty with
  ...
  | UserType (p, _, _, _) | History (p, _)  | Map (p, _, _) | Set (p, _)
