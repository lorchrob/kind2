...
let rec reconstruct_adt_at_step model bindings adt_map step root_index type_name =
  match LDAT.HStringMap.find_opt type_name adt_map with
  | None -> assert false
  | Some (adt_info : LDAT.adt_info) ->
    let disc_index =
      root_index @ [D.AdtTagIndex (HString.string_of_hstring adt_info.type_name)]
    in
    ...
    | Some disc_sv ->
      ...
      | Some (Model.Term t) when Type.is_enum disc_type ->
        ...
        let fields =
          match LDAT.HStringMap.find_opt ctor_hs adt_info.ctor_fields with
          | None -> assert false | Some fs -> fs
        in
        if fields = [] then ctor_name
        else
          let field_strs = List.mapi (fun j (_, ftype) ->
            let field_index =
              root_index @ [D.AdtPayloadIndex (ctor_name, j)]
            in
            (match LDAT.adt_info_of_type adt_map ftype with
            | Some sub_info ->
              reconstruct_adt_at_step model bindings adt_map step
                field_index sub_info.type_name
            | None ->
...
let adt_streams_from_bindings (adt_map : LDAT.adt_map) model node bindings =
  if LDAT.HStringMap.is_empty adt_map then []
  else
