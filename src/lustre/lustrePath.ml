...
(* Recursively reconstruct the ADT value string at a given step.
   root_index: the index prefix for this ADT variable's record (e.g. [] for top-level,
               [AdtPayloadIndex "Cons_1"] for a nested field).
   type_name:  the ADT type name (key into adt_map).
   disc_sv:    the discriminant state variable (passed directly to avoid ambiguous lookup).
...
let rec reconstruct_adt_at_step model bindings adt_map step root_index type_name disc_sv =
  ...
  | Some (adt_info : LDAT.adt_info) ->
    ...
    | Some (Model.Term t) when Type.is_enum disc_type ->
      ...
      let fields =
        match G.HStringMap.find_opt ctor_hs adt_info.ctor_fields with
        | None -> assert false | Some fs -> fs
      in
      if fields = [] then ctor_name
      else
        let field_strs = List.mapi (fun j (_fname, ftype) ->
          let field_index = root_index @ [D.AdtPayloadIndex (ctor_name, j)] in
          let nested_type_opt = match ftype with
            | LA.UserType (_, _, name) when G.HStringMap.mem name adt_map -> Some name
            | LA.ADT (_, name, _) when G.HStringMap.mem name adt_map -> Some name
            | _ -> None
          in
          match nested_type_opt with
          | Some nested_type ->
...
let adt_streams_from_bindings (adt_map : G.adt_map) model node bindings =
  if G.HStringMap.is_empty adt_map then []
    ...
    (* Collect top-level disc svs: Generated (Discriminant type_name) whose root_index
       contains no ADT payload indices (i.e., not nested inside a payload). *)
    ...
    List.filter_map (fun (_disc_index, disc_sv, type_name, root_index) ->
      ...
      let root_name =
        ...
        | None -> sv_name
        | Some (adt_info : LDAT.adt_info) ->
          let disc_suffix = "." ^ HString.string_of_hstring adt_info.disc_field in
