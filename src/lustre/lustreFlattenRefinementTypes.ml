...
let rec flatten_ref_types_expr: TypeCheckerContext.tc_context -> A.expr -> A.expr = 
  ...
  | StructUpdate (p, e1, i, Some e2, None) -> StructUpdate (p, rec_call e1, i, Some (rec_call e2), None)
  | StructUpdate (p, e1, i, Some e2, Some (ty1, ty2)) ->
    StructUpdate (p, rec_call e1, i, Some (rec_call e2), Some (flatten_ref_type ctx ty1, flatten_ref_type ctx ty2))
  | StructUpdate (p, e1, i, None, None) -> StructUpdate (p, rec_call e1, i, None, None)
  | StructUpdate (p, e1, i, None, Some (ty1, ty2)) ->
    StructUpdate (p, rec_call e1, i, None, Some (flatten_ref_type ctx ty1, flatten_ref_type ctx ty2)) 
